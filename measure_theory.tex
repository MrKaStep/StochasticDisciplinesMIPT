...
\begin{document}
  ...
  \section{Дискретное вероятностное пространство. Классическая вероятность. Построение простейших вероятностных пространств. Элементы комбинаторики. Вероятность суммы событий.}
	...
	\end{proof}
	\begin{corollary}$  $\\
		Если $ \nu $ — $ \sigma $-аддитивная мера на кольце $ R $ и $ A, A_{1}, \dots \in R,\ A \subseteq \bigcup_{i=1}^{\infty}{A_{i}} $, то $ \nu(A) \le \sum_{i=1}^{\infty}{\nu(A_{i})} $.
	\end{corollary}
	\begin{proof}
		...
		Тогда 
		\begin{gather*} 
			A := \bigsqcup\limits_{n=1}^{\infty} B_{n}\\
	...
	\begin{proof}
		...
		\[ \mu^{*}(A) \le \sum_{i=1}^{\infty}{\sum_{j=1}^{\infty}{m(A_{i, j})}} < \sum_{i=1}^{\infty}{\mu^{*}(A_{i})} + \eps \]
	...
	\begin{definition}
		...
		Как и мера Лебега, она $ \sigma $-аддитивна, однако её область определения \'{у}же в силу существования измеримых по Лебегу неборелевских множеств. В частности, мера Бореля необязательно полна. Пополнение меры Бореля — ещё один способ конструкции меры Лебега. 
	\end{definition}
	\begin{definition}
	...
	\begin{proof}
		...
		\end{gather*}\footnote{
			\begin{gather*} 
				\phi(b_{i}) - \phi(c_{i}) < \frac{\eps}{2^{i+1}} \Leftrightarrow \phi(b_{i}) < \phi(c_{i}) + \frac{\eps}{2^{i+1}}\Leftrightarrow\\\Leftrightarrow \phi(b_{i}) - \phi({a_{i}}) < (\phi(c_{i}) - \phi({a_{i}})) + \frac{\eps}{2^{i+1}} \Leftrightarrow m([a_{i}, b_{i})) < m([a_{i}, c_{i})) + \frac{\eps}{2^{i+1}} 
			\end{gather*}
		}
		...
		\begin{gather*} 
			...
			\le  \lim\limits_{n\to\infty}{\sum\limits_{i=1}^{\infty}m(A_{i} \cap [-n, b))} \le \sum\limits_{n=1}^{\infty}m(A_{i})
		\end{gather*}
	...
	\begin{proof}$  $
		\begin{itemize}
			\item $ \varphi^{-1}((c, +\infty]) = \{x \in X: \exists n\ (f_{n}(x) > c) \} =  \bigcup\limits_{n=1}^{\infty}{f^{-1}_{n}((c, +\infty])} \in \Sigma $\footnote{Если для данного $ x $ существует какое-то $ n $, такое что $ f_{n}(x) > c $, то $ \sup\limits_{n}f_{n}(x) > c $ и подавно.}
			\item \begin{gather*}  
					\psi^{-1}((c, +\infty]) = \{x \in X: \exists m\ \forall n \exists\ k \ge n\ (f_{k}(x) > c + \frac{1}{m}) \} =\\= \bigcup\limits_{k=1}^{\infty}{ \bigcap\limits_{n=1}^{\infty}{\bigcup\limits_{k=n}^{\infty}f^{-1}_{k}\left (\left (c + \frac{1}{k}, +\infty\right ]\right )  }} \in \Sigma
				\end{gather*}
		\end{itemize}
		Т.к. в определении верхнего предела фигурирует инфимум, отделение от $ c $ добавочным слагаемым $ \frac{1}{k} $ необходимо, т.к. без него не выйдет отфильтровать случай $ \psi(x) = c $.
	\end{proof}
	\begin{corollary}$  $\\
	...
	\section{Сходимость по мере и почти всюду. Их свойства (критерий Коши сходимости по мере, арифметические, связь сходимостей, Теорема Рисса).}
		...
		\begin{proof}
			...
			\begin{gather*} 
				\forall \eps > 0: \{x \in X : |f(x) - g(x)| > \eps \} \subseteq \{x \in X: |f(x) - f_{n}(x)| >  \eps/2 \} \cup \{x \in X: |g(x) - f_{n}(x)| > \eps/2 \} \implies\\
				\implies  \forall \eps > 0\ \forall \gamma > 0: \mu(\{|f - g| > \eps \}) < \frac{\gamma}{2} + \frac{\gamma}{2} < \gamma
			\end{gather*} 
		...
		\begin{proof}\footnote{Доказательство очень техническое, тяжело парсится. Стоит найти другое. Здесь было бы очень уместно добавить иллюстрацию, на которой будут указаны все участвующие в доказательстве множества.}$  $\\
			...
			\begin{gather*} 
				\forall n \ge N\ \forall x \in X \setminus (A \cup B_{n}): f(x) \in K \subset K',\ f_{n}(x) \in K',\ |f_{n}(x) - f(x)| < \delta \implies\\\implies
				|g(f_{n}(x)) - g(f(x))| < \eps 
			\end{gather*}
			 Значит, 
			 \begin{gather*}
				 \forall \eps > 0\ \forall \gamma > 0\ \exists N\ \forall n > N: \mu(\{ |g \circ f_{n} - g \circ f| > \eps \}) \le \gamma \implies\\
				 \implies g \circ f_{n} \xrightarrow{\mu} g \circ f
			 \end{gather*}
		\end{proof}
		\begin{corollary}
		...
		\begin{theorem}[Рисса]$  $\\
			...
			Тогда существует подпоследовательность $ f_{n_{i}} \xrightarrow{\text{п.в.}} f $.
		\end{theorem}
		\begin{proof}$  $\\
	...
	\section{Интеграл Лебега для конечно-простых функций и его свойства. Определение интеграла Лебега в общем случае. Основные свойства интеграла Лебега.}
		...
		\end{preposition}
		\begin{theorem}
			Пусть $ (X, \Sigma, \mu) $ — $ \sigma $-конечное измеримое пространство и $ E \in \Sigma $.\\
			Тогда если $ g_{n}: E \to \Real^{+} $ — неубывающая последовательность простых функций и\\ \[ g: E \to \Real^{+},\ g(x) := \sum\limits_{i=1}^{n} a_{i} \mathbf{I}_{E_{i}}(x),\ 0 < a_{1} < \dots < a_{n} ,\ \bigsqcup\limits_{i=1}^{n} E_{i} \subseteq E,\ \mu(E_{i}) < \infty \] — также простая функция, причём $ \forall x_{0} \in E: \lim\limits_{n \to \infty} g_{n}(x_{0}) \ge g(x_{0}) $, то 
		...
		\begin{proof}
			...
			\begin{itemize}
				...
				Если $ f(x_{0}) < +\infty $ (иначе тривиально), тогда $ \exists m_{0}: x_{0} \in \bigsqcup\limits_{n=1}^{m_{0}} E_{n},\ f(x_{0}) < 2^{m_{0}} $.\\ 
		...
		\begin{proof}
			...
			По определению интеграла Лебега, $ \int_{E} (f + g) d\mu = \int_{E} (f + g)_{+} d\mu - \int_{E} (f + g)_{-} d\mu  $.\\
			Разделим множество значений на $ E_{+} := \{ f + g \ge 0 \} $ и $ E_{-} := \{ f + g < 0 \} $.\\
			Вычислим по отдельности $ \int_{E_{+}} (f + g) d\mu,\ \int_{E_{-}} (f + g) d\mu $, поскольку эти функции знакопостоянны и весь необходимый аппарат для них разработан.\\
			По линейности интеграла Лебега от знакопостоянных функций (т. 19.3 и утв. 19.5) получаем, что 
			\begin{gather*}
			...
			Значит
			\begin{gather*}
				\int_{E}(f + g) d\mu = \int_{E}(f + g)_{+} d\mu - \int_{E}(f + g)_{-} d\mu = \int_{E_{+}}(f + g) d\mu + \int_{E_{-}}(f + g) d\mu =\\=  \int_{E_{+}} f d\mu + \int_{E_{+}} g d\mu + \int_{E_{-}} f d\mu + \int_{E_{-}} g d\mu = \int_{E} f d\mu + \int_{E} g d\mu
			\end{gather*}
	...
	\end{lemma}
	\begin{proof}
		Положим $ g_{n} := \inf\limits_{k \ge n} f_{k} $, тогда $ g_{n} \le f_{n},\ g_{n} \uparrow f $.\\
		Поскольку $ f_{n} \xrightarrow{\text{п.в.}} f $, она не сходится к $ f $ не на всём $ E $, а на таком $ E' \subseteq E $, что $ \mu(E \setminus E') = 0 $. Тогда
		\[ 
			\forall x \in E': \lim\limits_{n\to\infty} g_{n} = \liminf\limits_{n\to\infty} f_{n} = f 
		\]
	...
	\begin{theorem}[Лебега, \href{https://en.wikipedia.org/wiki/Dominated_convergence_theorem}{Dominated convergence theorem}]$  $\\
		Пусть $ \mu $ полна, $ f_{n} \xrightarrow{\text{п.в.}} f $ — такая посл-ть измеримых, что $ \exists F \in L(E)\ \forall n: |f_{n}| \le F $. Тогда
		\[ 
	...
	\begin{proof}$  $\\
		...
		\begin{gather*}
			\int_{A} f d\mu = \int_{A} f d\mu - \int_{A} h d\mu + \int_{A} h d\mu \le\\\le \int_{E} f d\mu - \int_{E} h d\mu + \int_{E} \sum_{i=1}^{n} (a_{k} \cdot \mbox{I}_{E_{i} \cap A}) d\mu <\\< \frac{\eps}{2} + \sum_{i=1}^{n} a_{k} \mu(E_{i} \cap A) \le \frac{\eps}{2} + \max\limits_{1 \le i \le n} a_{i} \sum\limits_{k=1}^{n} \mu(E_{k} \cap A) \le \frac{\eps}{2} + \max\limits_{1 \le k \le n} a_{k} \cdot \mu(A) < \eps
		\end{gather*}
	\end{proof}
